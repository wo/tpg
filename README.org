...
On the other hand, the complications do not arise for the simplified tableaux
rules of S5, where we can ignore the accessibility clauses. Here we also use the
full free-variables machinery.

Moreover, the translation simplifies the "model finder" (see below). 

I also hope to make better use of the free-variable machinery in the future. (At
a minimum: when applying the seriality rule, we could use a new world /variable/.)
