...
prover = {
	debug : true,
	fastMode : false, // fastMode uses advanced techniques like merging that can't be translated back into sentence tableaux, alsways turned off atm
	...
	nextStep : function() {
		...
		this.status("step " + this.steps + ": " + numBranches + " branches, " + this.tree.openBranches[0].nodes.length + " nodes and " + this.tree.openBranches[0].freeVariables.length + " vars on branch, " + this.alternatives.length + " alternatives, search depth " + this.depthLimit);
		...
		case tc.LITERAL : {
			...
			if (!node.unifyWith) {
				...
				for (var i=0; i<this.literals.length; i++) {
					var unifier = negatedFormula.unify(this.literals[i].formula);
					if (unifier.isArray) unifyCandidates.push(this.literals[i]);
				}
				if (unifyCandidates.length == 0) { // unification impossible
					this.literals.unshift(node);
					return 0;
				}
			...
			// now unify:
			var unifier = negatedFormula.unify(node.unifyWith.formula);
